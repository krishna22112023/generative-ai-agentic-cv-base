...
class ImageRestormer:
    ...
    def get_model(self):
        ...
        if self.task in model_urls:
            ...
            folder_path = os.path.join(root,'modules','Restormer','models',folder_paths[self.task])
    ...
    def load_model(self):
        ...
        # Get weights path
        weights_path = self.get_model()

        # Load the architecture
        try:
            load_arch = run_path(os.path.join(root, 'modules','Restormer','basicsr', 'models', 'archs', 'restormer_arch.py'))
...
def main(pipeline, input_dir, output_dir):
    ...
    for task in unique_tasks:
        ...
        model = restormer_obj.load_model()
        model_cache[task] = (restormer_obj, model)

    # Process each image
    for image, tasks in tqdm(pipeline.items(),desc="Processing images using restormer"):
        image_path = os.path.join(input_dir, image) #image is the relative path from input_dir
        img = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        for task in tasks:
            print(f"Processing image {image} with task {task}")
            restormer_obj, model = model_cache[task]
            img = restormer_obj.run_model(model, img)
        cv2.imwrite(os.path.join(output_dir, image), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
...
if __name__ == "__main__":
    ...
    if pipeline is None:
        logger.error(f"No preprocessing pipeline found. Skipping preprocessing.")
    else:
